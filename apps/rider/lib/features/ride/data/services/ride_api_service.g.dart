...
class _RideApiService implements RideApiService {
  ...
  Future<ReviewRideResponse> reviewRide(
    ...
    final _headers = <String, dynamic>{};
    final _data = <String, dynamic>{};
    _data.addAll(reviewRideBody.toJson());
    final _options = _setStreamType<ReviewRideResponse>(
      Options(method: 'POST', headers: _headers, extra: _extra)
          .compose(
            _dio.options,
            '/api/v1/Rides/${rideId}/review',
            queryParameters: queryParameters,
            data: _data,
            cancelToken: cancelToken,
          )
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late ReviewRideResponse _value;
    try {
      _value = ReviewRideResponse.fromJson(_result.data!);
    } on Object catch (e, s) {
      errorLogger?.logError(e, s, _options, response: _result);
      rethrow;
    }
